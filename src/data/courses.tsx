...
export const regexToFsmCourse: RegexToFsmLevel[] = [
  ...
  },
  {
    id: 2,
    regex: 'meow*',
    description: 'Постройте автомат для "meow*", принимающий "meo", "meow", "meoww" и т.д.',
    tutorial: [
      {
        ...
        content: (
          <>
            <p>
              <strong>Регулярное выражение</strong> — это компактная запись правила, по которому можно описать не одну конкретную строку, а целое множество допустимых строк.
            </p>
            <p>
              Если в первом уроке мы строили автомат для точного слова <code>mama</code>, то теперь правило станет гибче: часть строки сможет повторяться несколько раз или не появляться вовсе.
            </p>
            <p>
            ...
            <p>
              Например, выражение <code>ma*</code> задаёт строки, которые начинаются с обязательной буквы <code>m</code>, а затем содержат ноль или больше букв <code>a</code>: <code>m</code>, <code>ma</code>, <code>maa</code> и так далее.
            </p>
            <p>
              Важно: регулярное выражение описывает всю строку целиком. Поэтому <code>am</code> или <code>mma</code> для <code>ma*</code> не подходят: они нарушают порядок символов.
            </p>
          </>
        )
      },
      {
        title: 'Из чего состоят',
        content: (
          <>
            <p>
              Регулярные выражения бывают простыми, когда они описывают одну точную строку, и составными, когда в них используются <strong>специальные операторы</strong>.
            </p>
            <p>
              Читать выражение удобно слева направо: обычные символы должны встретиться именно в таком порядке, а операторы уточняют, сколько раз или каким способом эти символы используются.
            </p>
            <p>
              Из чего могут состоять регулярные выражения:
            </p>
            <ul className="tutorial-list">
              <li><strong>Символы</strong>, например <code>a</code> или <code>m</code>, обозначают сами себя и должны быть прочитаны автоматом.</li>
              <li><strong>Конкатенация</strong> — запись символов подряд. Например, <code>ma</code> означает: сначала <code>m</code>, затем <code>a</code>.</li>
              <li><strong>Операторы повторения</strong>, например <code>*</code>, <code>+</code> и <code>?</code>, задают количество повторов символов или групп.</li>
            ...
            <p style={{ marginTop: '12px' }}>
              В выражении <code>ma*</code> оператор <code>*</code> относится только к ближайшей букве слева — к <code>a</code>, а не ко всей части <code>ma</code>.
            </p>
          </>
        )
      },
      {
        ...
        content: (
          <>
            ...
            <p>
              Слово «ноль» здесь особенно важно: символ перед <code>*</code> можно вообще не читать. Поэтому <code>a*</code> принимает даже пустую строку.
            </p>
            <p>
      ...
      },
      {
        ...
        content: (
          <>
            <p>
              В конечном автомате повторение одного и того же символа реализуется с помощью <strong>петли</strong> — перехода из состояния в само себя.
            </p>
            <div style={{ display: 'flex', justifyContent: 'center', margin: '24px 0' }}>
  ...
  },
  {
    id: 3,
    regex: 'cat|dog',
    description: 'Постройте автомат, принимающий строки "cat" и "dog".',
    tutorial: [
      {
        title: 'Оператор ИЛИ (|)',
        content: (
          <>
            <p>
              <strong>Оператор ИЛИ (|)</strong> позволяет задать выбор между несколькими вариантами. Это эквивалентно логическому "ИЛИ".
            </p>
            <p>
